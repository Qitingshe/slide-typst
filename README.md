...
中文主字体为 **Noto Sans CJK SC**（SIL OFL 开源，替代 New Computer Modern + Heiti SC 回退链）；Linux CI 现通过 apt 安装 fonts-noto-cjk-extra 对齐字重覆盖（Thin…Black），但字体版本/度量差异仍可能带来分页漂移，属已知现象（见下文"门禁分层"）。
...
- **CI（ubuntu）**：页面检查降级为 warn —— ubuntu 经 apt 安装 fonts-noto-cjk-extra（字重覆盖与 macOS 一致），但字体版本/度量与 macOS 存在差异，分页漂移属预期，故不以 ubuntu 页数作硬闸；其余五项硬闸。
